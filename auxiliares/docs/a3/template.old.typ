...
#let rp-theme(aspect-ratio: "16-9", body) = {
  set text(fill: rp-colors.text, size: 30pt, font: "SF Pro")
  let background = {
    let p = pattern(
      box(
        inset: (top: 2pt), fill: rp-colors.base, text(fill: rp-colors.overlay, font: "SF Mono", size: 10pt, "10"),
      ),
    )
  ...
  polylux-slide(
    {
      ...
      v(-1em)
      text(
      fill: rp-colors.gold, weight: "bold", font: "Helvetica", size: 2.5em, title,
      )
      place(
        top + right,
      )[
  ...
  polylux-slide(
    {
      ...
      box(
        ...
        #set text(fill: rp-colors.rose, font: "SF Mono")
        \~/#context aux.course.get()/Aux#context aux.number.get()
        #set text(fill: rp-colors.text, font: "Helvetica", size: 2em)
        #v(-.5em)
        #box(
          fill: rp-colors.hl-low, width: 100%, height: 2em, stack(
            dir: ltr, spacing: 1em, text(font: "SF Mono", ">"), text(fill: rp-colors.text, underline(content)),
          ),
        )
      ]
    },
  ...
  polylux-slide(
    {
      ...
      align(
        center + horizon, box(
          ...
          #if title != [] {
            align(
              center + top, text(font: "Helvetica", weight: "bold", size: 1.5em, fill: rp-colors.rose, title),
            )
          }
          #content
        ],
